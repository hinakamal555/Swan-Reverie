(() => {
  ...
  window.InvitationTemplates.register("classic-floral-01", {
    ...
    render(root, invitation, options) {
      ...
      const heroSrc = safeUrl(content.media && content.media.heroImageUrl) || "/assets/hero-archway.jpg";
      ...
      const calendarLabel = text.calendarButtonLabel || ui.calendar;
      ...
      const eventsHtml = events.map((event) => {
        ...
        const palette = (Array.isArray(event.colors) && event.colors.length
          ? event.colors
          : (content.theme && content.theme.colors) || ["#c4a35a", "#7a8b68", "#f3ece1"]
        ).slice(0, 3);
        const dots = palette.map((color) => {
          const hex = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(color) ? color : "#f3ece1";
        ...
        return `
          <li class="event-card">
            ...
            ${(maps || event.date) ? `<div class="event-actions">
              ${maps ? `<a class="maps-btn" href="${escapeHtml(maps)}" target="_blank" rel="noopener noreferrer">
              ...
              ${event.date ? `<button type="button" class="maps-btn calendar-btn" data-event-id="${escapeHtml(event.id)}">${escapeHtml(calendarLabel)}</button>` : ""}
            </div>` : ""}
          </li>
        `;
      }).join("");
      ...
      root.innerHTML = `
        ${embedded ? "" : `
        <div class="gate-stage" id="gateStage">
          <div class="gatefold" id="gatefold" role="dialog" aria-modal="true" aria-label="Closed wedding invitation">
            ...
            <div class="gate gate--right door-right">
              ...
              <button class="seal-btn" id="openInvite" type="button" aria-label="${escapeHtml(ui.openSeal)}">
                <img class="seal-img" src="/assets/wax-seal.png" alt="" width="220" height="220" draggable="false">
              </button>
            </div>
          </div>
        </div>`}

        <div class="page invite-container" id="page">
          ...
          <header class="hero" id="hero">
            <div class="hero-art">
              <img src="${escapeHtml(heroSrc)}" alt="" width="1200" height="1600">
            </div>
            <div class="hero-copy">
          ...
          <main>
            <section class="verse intro" id="intro" aria-labelledby="intro-heading">
            ...
            <section class="when" id="when">
              <span class="fade-edge fade-edge--top" aria-hidden="true"></span>
              ...
              ${scratchEnabled ? `
                ...
                <p class="married-badge" id="marriedBadge" hidden>${escapeHtml(completion)}</p>
              ` : ""}
              <span class="fade-edge fade-edge--bot" aria-hidden="true"></span>
            </section>
            ...
            <section class="closing" id="closing">
              <span class="fade-edge fade-edge--top" aria-hidden="true"></span>
              <div class="urns">
                <picture><source srcset="/assets/floral-urn.webp" type="image/webp"><img class="urn urn--left" src="/assets/floral-urn.png" alt=""></picture>
                <picture><source srcset="/assets/floral-urn.webp" type="image/webp"><img class="urn urn--right" src="/assets/floral-urn.png" alt=""></picture>
              </div>
